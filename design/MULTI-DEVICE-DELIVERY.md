...
目标：保留现有 Mac 记录、待办、AI 和 CLI，在 Mac 与 iOS 之间交付可离线使用、支持账号隔离和冲突恢复的待办同步。采用 Supabase Auth / PostgreSQL + PowerSync；不依赖 iCloud 文件同步。

## 本次范围

- Mac 保留现有界面，设置增加服务配置、邮箱密码登录、退出、导入本机待办、同步状态和冲突恢复。
- iOS 17+ 原生 SwiftUI：待办增改、日期分组、搜索、重要筛选、完成、删除和同步设置。
...
共享 `NotoCore` 承担业务数据和迁移，`NotoSync` 承担认证、上传和下载。Mac/iOS 复用同一份 Swift 代码。
...
iOS 工程、模拟器构建和测试命令见 [iOS README](../iOS/README.md)。真机需配置自己的 Apple 开发团队，模拟器 `.app` 不能作为 iPhone 安装包。
...
| iOS 离线 UI | 模拟器新增、重启保留、修改、完成、删除 | 已通过 |
| Swift 真实同步链路 | 生产 Swift SDK、GRDB outbox、真实 Auth/RPC、两端冲突/恢复和第三账号隔离 | 已通过 |
| iOS 真实同步 UI | 登录、上传下载、退出和账号隔离 | 已通过 |
| iPhone 目标编译 | Release / iphoneos / arm64，无签名构建 | 已通过；未安装真机 |
| 公网与真机 | 指定云项目部署、Mac 与实体 iPhone、后台恢复、签名分发 | 尚未执行 |

尚未获得可写入的指定云项目确认和 Apple 签名/真机环境，因此本地验证不等同于云端上线。没有修改已有的远端 Supabase 项目。再次检查本机：`security find-identity -v -p codesigning` 返回 0 个有效签名身份；`xcrun devicectl list devices` 返回无设备。

交付产物：`build/Noto.app`、`build/bin/noto`、`build/ios/NotoIOS.app`（仅模拟器）。另提供 `build/ios-device-unsigned/NotoIOS.app`（iPhone 目标 Release 编译产物，未签名，不能直接安装）。本次构建和测试日志保存在 `build/verification/`。iOS 真实界面验证可用 `iOS/run-live-tests.sh` 重跑。

## 云端上线验收

1. 指定 Supabase/PowerSync 测试环境，部署迁移与同步规则，分别创建两个测试账号。检查客户端仅包含公开配置。
2. Mac 与实体 iPhone 登录同一账号：一端新增、改日期、改优先级、完成，另一端前台连接后收敛。记录实际同步延迟。
...
6. iPhone 前后台切换、弱网、令牌过期和系统终止后恢复同步。后台时间由 iOS 控制，本版本不承诺后台实时同步。
7. 对历史数据先备份再迁移，验证导入幂等；运行大数据量交互与首轮同步检查；完成签名、公证/TestFlight 和备份恢复演练。
